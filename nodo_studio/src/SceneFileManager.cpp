...
void SceneFileManager::openScene() {
  ...
  if (loaded_graph.has_value()) {
    ...
    // Update the node graph widget to reflect the loaded graph
    if (node_graph_widget_) {
      node_graph_widget_->set_graph(node_graph_);
    }
...
bool SceneFileManager::saveScene() {
  if (!node_graph_)
    return false;
...
bool SceneFileManager::saveSceneAs() {
  if (!node_graph_)
    return false;
...
void SceneFileManager::exportGeometry() {
  if (!node_graph_ || !execution_engine_)
    return;
