...
int main() {
  ...
  if (sphere_result && box_result) {
    ...
    std::cout << "✓ Generated sphere: " << sphere_mesh->vertices().rows()
              << " vertices\n";
    std::cout << "✓ Generated box: " << box_mesh->vertices().rows()
              << " vertices\n";
    ...
    if (union_result) {
      ...
      std::cout
          << "✗ Union operation failed (expected due to mesh closure issues)\n";
    }
  }
  ...
  auto cylinder_result =
      gpu::GPUMeshGenerator::generate_cylinder(0.5, 2.0, 8, 4);
  if (cylinder_result) {
    auto cylinder_mesh =
        std::make_shared<core::Mesh>(std::move(*cylinder_result));
    ...
    if (mirror_result) {
      std::cout << "✓ Mirror operation successful: "
                << mirror_result->vertices().rows() << " vertices\n";
  ...
  std::cout
      << "✓ Generated test files: test_boolean_union.obj, test_mirror_yz.obj\n";
