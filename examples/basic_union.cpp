...
int main() {
  try {
    ...
    auto box2 = geometry::MeshGenerator::box(Eigen::Vector3d(1, 1, 1),
    ...
    if (!val1) {
      std::cerr << "Box1 validation failed: "
                << geometry::BooleanOps::last_error().description() << "\n";
      return 1;
    }
    if (!val2) {
      std::cerr << "Box2 validation failed: "
                << geometry::BooleanOps::last_error().description() << "\n";
      return 1;
    }
    ...
    if (result) {
      ...
      std::cout << "Surface area: " << result->surface_area()
                << " square units\n";
      ...
      if (io::ObjExporter::export_mesh(*result,
                                       "examples/output/union_result.obj")) {
