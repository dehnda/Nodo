 ...
 * 
 * Shows ArraySOP, NoiseDisplacementSOP, and SubdivisionSOP as reusable components
 */
int main() {
    ...
    try {
        ...
        auto sphere_opt = gpu::GPUMeshGenerator::generate_sphere(1.0f, 32, 16);
        if (!sphere_opt) {
        ...
        linear_array.set_linear_offset(Eigen::Vector3f(3.0f, 0.0f, 0.0f));
        ...
        if (linear_result) {
            ...
            exporter.export_mesh(*linear_result, "sop_linear_array.obj");
        ...
        radial_array.set_count(6);
        radial_array.set_radial_radius(4.0f);
        radial_array.set_angle_step(60.0f);
        ...
        if (radial_result) {
            ...
            io::ObjExporter exporter;
            exporter.export_mesh(*radial_result, "sop_radial_array.obj");
        ...
        noise_displacement.set_amplitude(0.3f);
        noise_displacement.set_frequency(2.0f);
        noise_displacement.set_octaves(4);
        noise_displacement.set_seed(42);
        ...
        if (noise_result) {
            ...
            io::ObjExporter exporter;
            exporter.export_mesh(*noise_result, "sop_noise_displacement.obj");
        ...
        if (subdivision_result) {
            std::cout << "✓ Subdivision: " << subdivision_result->vertices().rows() 
                      << " vertices, " << subdivision_result->faces().rows() << " faces\n";
            
            io::ObjExporter exporter;
            exporter.export_mesh(*subdivision_result, "sop_subdivision.obj");
        ...
        io::ObjExporter exporter;
        exporter.export_mesh(*step3, "sop_complex_pipeline.obj");
        ...
        std::cout << "• sop_complex_pipeline.obj - Noise->Subdivision->Array pipeline\n\n";
