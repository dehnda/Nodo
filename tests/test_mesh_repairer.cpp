...
TEST_F(MeshRepairerTest, ToleranceSettings) {
    ...
    auto result_strict = geometry::MeshRepairer::merge_duplicate_vertices(test_mesh1, 1e-12);
    auto result_loose = geometry::MeshRepairer::merge_duplicate_vertices(test_mesh2, 1e-6);
...
TEST_F(MeshRepairerTest, CleanMeshUnchanged) {
    ...
    // Box generator may create duplicate vertices that get merged
    // So vertices may be reduced, but faces should stay the same or decrease
