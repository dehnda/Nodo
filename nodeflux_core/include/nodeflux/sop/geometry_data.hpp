 ...
 * workflow. It can hold meshes, point clouds, curves, and associated
 * attributes.
 */
class GeometryData {
public:
...
public:
   ...
   */
  std::optional<AttributeArray>
  get_vertex_attribute(const std::string &name) const {
    auto attr_it = vertex_attributes_.find(name);
    return attr_it != vertex_attributes_.end()
               ? std::make_optional(attr_it->second)
               : std::nullopt;
  }
   ...
   */
  void set_vertex_attribute(const std::string &name,
   ...
   */
  std::optional<AttributeArray>
  get_face_attribute(const std::string &name) const {
    auto attr_it = face_attributes_.find(name);
    return attr_it != face_attributes_.end()
               ? std::make_optional(attr_it->second)
               : std::nullopt;
  }
   ...
   */
  void set_face_attribute(const std::string &name,
                          const AttributeArray &values) {
   ...
   */
  void set_global_attribute(const std::string &name,
