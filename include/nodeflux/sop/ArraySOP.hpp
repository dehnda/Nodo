 ...
 * 
 * Creates multiple copies of input geometry arranged in linear, radial, or grid patterns.
 * This is a simplified version that works with the existing mesh system.
 */
class ArraySOP {
public:
...
public:
    ...
    void set_linear_offset(const Eigen::Vector3f& offset) { linear_offset_ = offset; }
    
    // Radial array configuration
    void set_radial_center(const Eigen::Vector3f& center) { radial_center_ = center; }
    ...
    void set_grid_size(int width, int height) { grid_size_ = Eigen::Vector2i(width, height); }
