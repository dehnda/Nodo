 ...
 * 
 * Subdivides mesh faces to create smoother surfaces with higher polygon density.
 */
...
public:
    explicit SubdivisionSOP(std::string name = "subdivision") : name_(std::move(name)) {}
    ...
    void set_preserve_boundaries(bool preserve) { preserve_boundaries_ = preserve; }
...
private:
    std::optional<core::Mesh> apply_catmull_clark_subdivision(const core::Mesh& mesh);
};
