...
public:
    AddNodeCommand(NodeGraphWidget* widget, nodeflux::graph::NodeGraph* graph,
                   nodeflux::graph::NodeType type, const QPointF& position)
        : Command("Add Node")
        , widget_(widget)
        , graph_(graph)
        , node_type_(type)
...
private:
    ...
    nodeflux::graph::NodeGraph* graph_;
...
public:
    DeleteNodeCommand(NodeGraphWidget* widget, nodeflux::graph::NodeGraph* graph, int node_id)
        : Command("Delete Node")
        , widget_(widget)
        , graph_(graph)
        , node_id_(node_id)
...
private:
    ...
    QString node_name_;
    QPointF position_;
    std::vector<nodeflux::graph::NodeParameter> parameters_;
    std::vector<nodeflux::graph::NodeConnection> connections_;
};
...
public:
    MoveNodeCommand(nodeflux::graph::NodeGraph* graph, int node_id,
...
public:
    ChangeParameterCommand(nodeflux::graph::NodeGraph* graph, int node_id,
    ...
    bool can_merge_with(const Command* other) const override {
        ...
        return (other_param != nullptr) &&
               (other_param->node_id_ == node_id_) &&
...
private:
    ...
    nodeflux::graph::NodeParameter old_value_;
    nodeflux::graph::NodeParameter new_value_;
};
...
public:
    ConnectCommand(NodeGraphWidget* widget, nodeflux::graph::NodeGraph* graph,
...
public:
    DisconnectCommand(NodeGraphWidget* widget, nodeflux::graph::NodeGraph* graph,
                     int connection_id)
