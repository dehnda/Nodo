...
void NodeCreationMenu::setupUI() {
    ...
    connect(search_box_, &QLineEdit::textChanged,
            this, &NodeCreationMenu::onSearchTextChanged);
    connect(results_list_, &QListWidget::itemClicked,
            this, &NodeCreationMenu::onItemClicked);
    connect(results_list_, &QListWidget::itemDoubleClicked,
            this, &NodeCreationMenu::onItemDoubleClicked);
...
void NodeCreationMenu::populateAllNodes() {
    ...
    // Generators (6 nodes)
    all_nodes_.append({"Sphere", "sphere_sop", "Generator", "●", {"primitive", "sphere", "uv"}});
    all_nodes_.append({"Box", "box_sop", "Generator", "■", {"primitive", "cube", "box"}});
    all_nodes_.append({"Cylinder", "cylinder_sop", "Generator", "▮", {"primitive", "cylinder"}});
    all_nodes_.append({"Plane", "plane_sop", "Generator", "▬", {"primitive", "plane", "grid"}});
    ...
    all_nodes_.append({"Export", "export_sop", "IO", "💾", {"export", "save", "write", "obj"}});
    ...
    all_nodes_.append({"Resample", "resample_sop", "Modifier", "◈", {"resample", "refine"}});
    all_nodes_.append({"Extrude", "extrude_sop", "Modifier", "↑", {"extrude", "offset"}});
    all_nodes_.append({"PolyExtrude", "polyextrude_sop", "Modifier", "⇈", {"extrude", "polygon", "face"}});

    // Arrays & Copies (3 nodes)
    all_nodes_.append({"Array", "array_sop", "Array", "⋮", {"array", "duplicate", "copy"}});
...
void NodeCreationMenu::filterResults(const QString& query) {
    ...
    if (query.isEmpty()) {
        ...
        if (!recent_nodes_.isEmpty()) {
            ...
            for (const NodeInfo& node : recent_nodes_) {
                QListWidgetItem* item = new QListWidgetItem(
                    QString("%1 %2").arg(node.icon, node.name));
        ...
        for (const NodeInfo& node : all_nodes_) {
            if (fuzzyMatch(query, node.name) ||
                fuzzyMatch(query, node.type_id) ||
