...
std::optional<core::Mesh> ArraySOP::create_linear_array(const core::Mesh& input_mesh) {
    ...
    for (int i = 0; i < count_; ++i) {
        ...
        Eigen::Vector3d current_offset = (linear_offset_ * static_cast<float>(i)).cast<double>();
...
std::optional<core::Mesh> ArraySOP::create_radial_array(const core::Mesh& input_mesh) {
    const auto& input_vertices = input_mesh.vertices();
    const auto& input_faces = input_mesh.faces();
    ...
    for (int i = 0; i < count_; ++i) {
        ...
        if (radial_radius_ > 0.0F) {
            position_offset += Eigen::Vector3d(
                radial_radius_ * std::cos(angle_rad),
                radial_radius_ * std::sin(angle_rad),
                0.0
            );
        }
...
std::optional<core::Mesh> ArraySOP::create_grid_array(const core::Mesh& input_mesh) {
    const auto& input_vertices = input_mesh.vertices();
    const auto& input_faces = input_mesh.faces();
