...
LaplacianSOP::LaplacianSOP(const std::string& name)
    ...
    , mu_(DEFAULT_MU)
    , method_(SmoothingMethod::UNIFORM)
    , preserve_boundaries_(DEFAULT_PRESERVE_BOUNDARIES)
    , cache_valid_(false)
    , last_input_hash_(0) {
}
...
std::shared_ptr<core::Mesh> LaplacianSOP::cook() {
    ...
    if (cached_result_) {
        ...
        std::cout << "  Result: " << cached_result_->vertices().rows() 
                  << " vertices, " << cached_result_->faces().rows() 
                  << " faces\n";
        cache_valid_ = true;
        ...
        if (input_mesh_) {
            ...
            last_input_hash_ = hasher(input_mesh_->vertices().rows() * 
                                    input_mesh_->faces().rows());
        }
    } else {
...
Eigen::MatrixXd LaplacianSOP::apply_uniform_laplacian(const Eigen::MatrixXd& vertices,
...
Eigen::MatrixXd LaplacianSOP::apply_cotangent_laplacian(const Eigen::MatrixXd& vertices,
                                                       const Eigen::MatrixXi& faces) {
    Eigen::MatrixXd result = vertices;
    auto boundary_vertices = find_boundary_vertices(faces, vertices.rows());
...
Eigen::MatrixXd LaplacianSOP::apply_taubin_smoothing(const Eigen::MatrixXd& vertices,
                                                    const Eigen::MatrixXi& faces) {
    Eigen::MatrixXd result = vertices;
    auto boundary_vertices = find_boundary_vertices(faces, vertices.rows());
...
Eigen::SparseMatrix<double> LaplacianSOP::build_uniform_laplacian(const Eigen::MatrixXi& faces,
                                                                 int num_vertices) {
...
Eigen::SparseMatrix<double> LaplacianSOP::build_cotangent_laplacian(const Eigen::MatrixXd& vertices,
                                                                   const Eigen::MatrixXi& faces) {
...
std::vector<bool> LaplacianSOP::find_boundary_vertices(const Eigen::MatrixXi& faces,
                                                       int num_vertices) {
...
double LaplacianSOP::calculate_cotangent_weight(const Eigen::Vector3d& v1,
...
bool LaplacianSOP::needs_recalculation() const {
    if (!input_mesh_) return true;
    
    std::hash<size_t> hasher;
    size_t current_hash = hasher(input_mesh_->vertices().rows() * 
                                input_mesh_->faces().rows());
