...
void NodeGraphEditor::initialize() {
    if (!is_initialized_) {
        ...
        style.Colors[ImNodesCol_NodeBackgroundSelected] = IM_COL32(100, 100, 100, 255);
...
void NodeGraphEditor::render() {
    ...
    if (auto_execute_) {
        ...
        bool needs_execution = std::any_of(nodes_.begin(), nodes_.end(),
...
void NodeGraphEditor::remove_node(int node_id) {
    ...
    links_.erase(std::remove_if(links_.begin(), links_.end(),
        [node_id](const NodeLink& link) {
            return link.get_start_node_id() == node_id || link.get_end_node_id() == node_id;
        }), links_.end());
...
void NodeGraphEditor::handle_interactions() {
    ...
    try {
        ...
        if (num_selected_nodes > 0 && num_selected_nodes < 100) { // Safety check
            ...
            for (auto& node : nodes_) {
                node.selected = std::find(selected_nodes.begin(), selected_nodes.end(), node.id) != selected_nodes.end();
            }
        }
    } catch (...) {
...
void NodeGraphEditor::execute_graph() {
    ...
    // This is a simplified implementation - real systems would use topological sort
        ...
        switch (node.type) {
            ...
            case NodeType::Smooth: {
                ...
                if (input_mesh) {
                    ...
                    smooth_sop.set_lambda(node.distance); // Reusing distance field for lambda
...
std::shared_ptr<core::Mesh> NodeGraphEditor::get_node_output(int node_id) const {
