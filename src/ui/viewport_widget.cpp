...
ViewportWidget::~ViewportWidget() { shutdown(); }
...
void ViewportWidget::render() {
  ...
  if (color_texture_ != 0) {
    ImGui::Image(
        reinterpret_cast<void *>(static_cast<intptr_t>(color_texture_)),
        ImVec2(static_cast<float>(framebuffer_width_),
...
void ViewportWidget::handle_mouse_input() {
  ...
  const ImVec2 mouse_delta = ImVec2(current_mouse_pos.x - last_mouse_pos_.x,
  ...
  if (ImGui::IsMouseDown(ImGuiMouseButton_Left) && is_hovered_) {
    if (!mouse_dragging_) {
      ...
      // Orbit camera
      viewport_renderer_.get_camera().orbit(mouse_delta.x * MOUSE_SENSITIVITY,
  ...
  if ((ImGui::IsMouseDown(ImGuiMouseButton_Middle) ||
    ...
    if (!mouse_panning_) {
      ...
      viewport_renderer_.get_camera().pan(mouse_delta.x * PAN_SENSITIVITY,
...
void ViewportWidget::render_viewport_controls() {
  ...
  const char *render_mode_names[] = {"Wireframe", "Solid", "Solid+Wire",
                                     "Normals"};
...
void ViewportWidget::update_from_execution_results() {
  ...
  for (const auto &pair : results) {
    if (pair.second) {
      viewport_renderer_.add_mesh(*pair.second,
                                  "Node " + std::to_string(pair.first));
    }
  }
}
...
void ViewportManager::set_execution_engine(
    std::shared_ptr<graph::ExecutionEngine> engine) {
