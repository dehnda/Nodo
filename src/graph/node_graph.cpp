...
void GraphNode::set_parameter(const std::string& name, const NodeParameter& param) {
    add_parameter(param);
}
...
void GraphNode::setup_pins_for_type() {
    ...
    switch (type_) {
        case NodeType::Sphere:
            ...
            parameters_.emplace_back("subdivisions", 16);
        ...
        case NodeType::Boolean:
            parameters_.emplace_back("operation", 0); // 0=Union, 1=Intersection, 2=Difference
            input_pins_.push_back({NodePin::Type::Input, NodePin::DataType::Mesh, "A", 0});
            input_pins_.push_back({NodePin::Type::Input, NodePin::DataType::Mesh, "B", 1});
            output_pins_.push_back({NodePin::Type::Output, NodePin::DataType::Mesh, "Result", 0});
            break;
            
        default:
            // Default: single input/output
            input_pins_.push_back({NodePin::Type::Input, NodePin::DataType::Mesh, "Input", 0});
            output_pins_.push_back({NodePin::Type::Output, NodePin::DataType::Mesh, "Output", 0});
            break;
    }
}
...
int NodeGraph::add_connection(int source_node_id, int source_pin, int target_node_id, int target_pin) {
    ...
    auto existing = std::find_if(connections_.begin(), connections_.end(),
