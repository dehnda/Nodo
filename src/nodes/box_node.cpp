...
BoxNode::BoxNode(double width, double height, double depth, int width_segments,
                 int height_segments, int depth_segments)
    : width_(width), height_(height), depth_(depth),
      width_segments_(width_segments), height_segments_(height_segments),
      depth_segments_(depth_segments), use_bounds_(false),
      min_corner_(Eigen::Vector3d::Zero()),
      max_corner_(Eigen::Vector3d::Zero()) {}

BoxNode BoxNode::create_from_bounds(const Eigen::Vector3d &min_corner,
...
std::optional<core::Mesh> BoxNode::generate() const {
  if (use_bounds_) {
    return geometry::BoxGenerator::generate_from_bounds(
        min_corner_, max_corner_, width_segments_, height_segments_,
        depth_segments_);
  } else {
    return geometry::BoxGenerator::generate(width_, height_, depth_,
                                            width_segments_, height_segments_,
                                            depth_segments_);
  }
}

void BoxNode::set_bounds(const Eigen::Vector3d &min_corner,
                         const Eigen::Vector3d &max_corner) {
  use_bounds_ = true;
