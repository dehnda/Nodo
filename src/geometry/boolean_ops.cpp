...
std::optional<core::Mesh> BooleanOps::cgal_boolean_operation(
    const core::Mesh& a, 
    const core::Mesh& b, 
    int operation_type) {
    ...
    try {
        ...
        for (int i = 0; i < a.faces().rows(); ++i) {
            faces_a.push_back({
                static_cast<std::size_t>(a.faces()(i, 0)),
        ...
        CGAL::Polygon_mesh_processing::polygon_soup_to_polygon_mesh(points_a, faces_a, cgal_a);
        ...
        for (int i = 0; i < b.faces().rows(); ++i) {
            faces_b.push_back({
                static_cast<std::size_t>(b.faces()(i, 0)),
