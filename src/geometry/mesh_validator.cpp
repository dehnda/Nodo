...
std::string ValidationReport::summary() const {
    ...
    if (!degenerate_face_indices.empty()) {
        ...
        if (degenerate_face_indices.size() > MAX_LISTED_ITEMS) oss << "...";
        oss << "\n";
    }
    ...
    if (!duplicate_vertex_indices.empty()) {
        ...
        if (duplicate_vertex_indices.size() > MAX_LISTED_ITEMS) oss << "...";
        oss << "\n";
    }
    ...
    if (!unreferenced_vertex_indices.empty()) {
        ...
        if (unreferenced_vertex_indices.size() > MAX_LISTED_ITEMS) oss << "...";
        oss << "\n";
    }
...
std::vector<int> MeshValidator::find_non_manifold_edges(const core::Mesh& mesh) {
    ...
    // Find faces adjacent to non-manifold edges (edges shared by more than 2 faces)
    ...
    non_manifold_faces.erase(std::unique(non_manifold_faces.begin(), non_manifold_faces.end()), 
