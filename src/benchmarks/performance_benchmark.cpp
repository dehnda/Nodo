#include "../../include/nodeflux/benchmarks/performance_benchmark.hpp"
#include "../../include/nodeflux/geometry/mesh_generator.hpp"
#include "../../include/nodeflux/spatial/enhanced_boolean_ops.hpp"
...
namespace nodeflux::benchmarks {

// PerformanceBenchmark Implementation
PerformanceBenchmark::PerformanceBenchmark(const BenchmarkConfig& config) 
    : config_(config) {}

const PerformanceBenchmark::BenchmarkResult* PerformanceBenchmark::BenchmarkSuite::find_result(
...
std::string PerformanceBenchmark::BenchmarkSuite::generate_report() const {
    ...
    for (const auto& result : results) {
        report << std::setw(30) << result.operation_name
               << std::setw(12) << result.average_time_ms
...
void PerformanceBenchmark::BenchmarkSuite::export_csv(const std::string& filename) const {
    ...
    // CSV header
    file << "Operation,Average_ms,Min_ms,Max_ms,StdDev_ms,Iterations,Memory_bytes,Additional_Info\n";
...
PerformanceBenchmark::BenchmarkSuite PerformanceBenchmark::run_bvh_benchmarks() {
    ...
    for (auto complexity : config_.complexity_levels) {
        ...
        switch (complexity) {
            case ComplexityLevel::Simple: complexity_name = "Simple"; break;
            case ComplexityLevel::Medium: complexity_name = "Medium"; break;
            case ComplexityLevel::Complex: complexity_name = "Complex"; break;
            case ComplexityLevel::VeryComplex: complexity_name = "VeryComplex"; break;
        }
        ...
        auto build_timings = time_function([&]() {
            bvh.build(test_mesh, params);
        }, config_.iterations);
        
        auto build_result = calculate_statistics(build_timings, 
            "BVH_Build_" + complexity_name);
        ...
        auto ray_timings = time_function([&]() {
            bvh.intersect_ray(test_ray);
        }, config_.iterations);
        
        auto ray_result = calculate_statistics(ray_timings, 
            "BVH_RayIntersect_" + complexity_name);
        suite.results.push_back(ray_result);
        
        // Benchmark AABB query
        spatial::AABB query_aabb(
            Eigen::Vector3d(-0.5, -0.5, -0.5),
            Eigen::Vector3d(0.5, 0.5, 0.5)
        );
        
        auto aabb_timings = time_function([&]() {
            bvh.query_aabb(query_aabb);
        }, config_.iterations);
        
        auto aabb_result = calculate_statistics(aabb_timings, 
            "BVH_AABBQuery_" + complexity_name);
...
PerformanceBenchmark::BenchmarkSuite PerformanceBenchmark::run_bvh_comparison_benchmarks() {
    ...
    for (auto complexity : config_.complexity_levels) {
        ...
        auto bvh_ray_timings = time_function([&]() {
            bvh.intersect_ray(test_ray);
        }, config_.iterations);
        
        auto bvh_ray_result = calculate_statistics(bvh_ray_timings, 
            "BVH_RayIntersect_" + complexity_name);
        ...
        }, std::min(config_.iterations, static_cast<size_t>(10))); // Fewer iterations for brute force
        
        auto brute_ray_result = calculate_statistics(brute_ray_timings, 
            "BruteForce_RayIntersect_" + complexity_name);
        ...
        auto bvh_aabb_timings = time_function([&]() {
            bvh.query_aabb(query_aabb);
        }, config_.iterations);
        
        auto bvh_aabb_result = calculate_statistics(bvh_aabb_timings, 
            "BVH_AABBQuery_" + complexity_name);
        suite.results.push_back(bvh_aabb_result);
        
        // Benchmark brute force AABB query
        auto brute_aabb_timings = time_function([&]() {
            brute_force_aabb_query(test_mesh, query_aabb);
        }, std::min(config_.iterations, static_cast<size_t>(10)));
        
        auto brute_aabb_result = calculate_statistics(brute_aabb_timings, 
            "BruteForce_AABBQuery_" + complexity_name);
...
PerformanceBenchmark::BenchmarkSuite PerformanceBenchmark::run_boolean_benchmarks() {
    ...
    for (auto complexity : config_.complexity_levels) {
        ...
        }, std::min(config_.iterations / 10, static_cast<size_t>(5))); // Boolean ops are expensive
        
        auto enhanced_union_result = calculate_statistics(enhanced_union_timings, 
            "Enhanced_Union_" + complexity_name);
...
PerformanceBenchmark::BenchmarkSuite PerformanceBenchmark::run_parameter_optimization_benchmarks() {
    ...
    for (int leaf_size : leaf_sizes) {
        ...
        auto result = calculate_statistics(build_timings, 
            "BVH_Build_Leaf" + std::to_string(leaf_size));
    ...
    auto sah_timings = time_function([&]() {
        bvh_sah.build(test_mesh, sah_params);
    }, config_.iterations / 2);
    ...
    auto midpoint_timings = time_function([&]() {
        bvh_midpoint.build(test_mesh, midpoint_params);
    }, config_.iterations / 2);
...
core::Mesh PerformanceBenchmark::create_test_mesh(size_t triangle_count) {
    // Create a sphere with appropriate subdivision level to reach target triangle count
    int subdivisions = 0;
    ...
    auto sphere_opt = geometry::MeshGenerator::sphere(
        Eigen::Vector3d::Zero(), 1.0, subdivisions);
    ...
    return geometry::MeshGenerator::box(
        Eigen::Vector3d(-1, -1, -1), 
        Eigen::Vector3d(1, 1, 1));
}

std::vector<double> PerformanceBenchmark::time_function(
...
PerformanceBenchmark::BenchmarkResult PerformanceBenchmark::calculate_statistics(
    const std::vector<double>& timings, const std::string& operation_name) {
...
std::optional<spatial::BVH::RayHit> PerformanceBenchmark::brute_force_ray_intersect(
    const core::Mesh& mesh, const spatial::BVH::Ray& ray) {
...
std::vector<int> PerformanceBenchmark::brute_force_aabb_query(
    const core::Mesh& mesh, const spatial::AABB& aabb) {
