...
Eigen::Vector3d AABB::size() const {
...
std::unique_ptr<BVHNode> BVH::build_recursive(
    ...
    if (triangle_indices.size() <= static_cast<size_t>(params_.max_triangles_per_leaf) || 
    ...
    // Split triangles
    auto [left_triangles, right_triangles] = params_.use_sah ?
        split_triangles_sah(triangle_indices, node_bounds) :
...
std::pair<std::vector<int>, std::vector<int>> BVH::split_triangles_sah(
    const std::vector<int>& triangle_indices,
    const AABB& node_bounds
) const {
...
std::pair<std::vector<int>, std::vector<int>> BVH::split_triangles_midpoint(
    const std::vector<int>& triangle_indices,
    const AABB& node_bounds
) const {
    // Find longest axis
    ...
    int longest_axis = 0;
    if (extent.y() > extent.x()) longest_axis = 1;
    if (extent.z() > extent[longest_axis]) longest_axis = 2;
