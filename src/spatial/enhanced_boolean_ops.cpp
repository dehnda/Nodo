...
#include "../../include/nodeflux/geometry/mesh_validator.hpp"
#include "../../include/nodeflux/geometry/mesh_repairer.hpp"
...
std::optional<core::Mesh> EnhancedBooleanOps::union_meshes(
    const core::Mesh& mesh_a, 
    ...
    if (!prepared_a || !prepared_b) {
        last_error_ = core::ErrorUtils::make_error(
            ...
            "Failed to prepare meshes for boolean operation", "EnhancedBooleanOps::union_meshes");
        return std::nullopt;
    }
...
std::optional<core::Mesh> EnhancedBooleanOps::intersect_meshes(
    const core::Mesh& mesh_a, 
    const core::Mesh& mesh_b,
    const BooleanParams& params
) {
    ...
    if (!prepared_a || !prepared_b) {
        last_error_ = core::ErrorUtils::make_error(
            core::ErrorCategory::Geometry, core::ErrorCode::InvalidMesh,
            "Failed to prepare meshes for boolean operation", "EnhancedBooleanOps::intersect_meshes");
        return std::nullopt;
    }
...
std::optional<core::Mesh> EnhancedBooleanOps::subtract_meshes(
    const core::Mesh& mesh_a, 
    const core::Mesh& mesh_b,
    const BooleanParams& params
) {
    ...
    if (!prepared_a || !prepared_b) {
        last_error_ = core::ErrorUtils::make_error(
            core::ErrorCategory::Geometry, core::ErrorCode::InvalidMesh,
            "Failed to prepare meshes for boolean operation", "EnhancedBooleanOps::subtract_meshes");
        return std::nullopt;
    }
...
std::vector<Eigen::Vector3d> EnhancedBooleanOps::find_intersection_points(
    const core::Mesh& mesh_a, 
    const core::Mesh& mesh_b
) {
...
std::optional<core::Mesh> EnhancedBooleanOps::prepare_mesh_for_boolean(
    ...
    if (params.validate_input) {
        ...
        if (!validation_result.is_valid) {
            ...
            if (!repair_result.success) {
                last_error_ = core::ErrorUtils::make_error(
                    core::ErrorCategory::Geometry, core::ErrorCode::InvalidMesh,
                    "Failed to repair mesh for boolean operation", "EnhancedBooleanOps::prepare_mesh_for_boolean");
                return std::nullopt;
            }
        }
    }
...
std::optional<core::Mesh> EnhancedBooleanOps::post_process_result(
    const core::Mesh& result,
    const BooleanParams& params
) {
    ...
    if (!repair_result.success) {
        last_error_ = core::ErrorUtils::make_error(
            core::ErrorCategory::Geometry, core::ErrorCode::InvalidMesh,
            "Failed to post-process boolean operation result", "EnhancedBooleanOps::post_process_result");
        return std::nullopt;
    }
