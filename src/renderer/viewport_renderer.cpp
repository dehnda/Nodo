...
Camera::Camera()
    : target_(0.0F, 0.0F, 0.0F), distance_(5.0F), yaw_(0.0F), pitch_(30.0F),
      fov_degrees_(45.0F), near_plane_(0.1F), far_plane_(100.0F) {}
...
void Camera::pan(float delta_x, float delta_y) {
  ...
  const Eigen::Vector3f right(std::cos(yaw_rad + M_PI / 2.0F), 0.0F,
...
Eigen::Matrix4f Camera::get_view_matrix() const {
  ...
  view(0, 1) = right.y();
  view(0, 2) = right.z();
  view(1, 0) = camera_up.x();
  view(1, 1) = camera_up.y();
  view(1, 2) = camera_up.z();
  view(2, 0) = -forward.x();
  view(2, 1) = -forward.y();
  view(2, 2) = -forward.z();
...
MeshRenderData::~MeshRenderData() { cleanup(); }

MeshRenderData::MeshRenderData(MeshRenderData &&other) noexcept
    : vao_(other.vao_), vertex_vbo_(other.vertex_vbo_),
      normal_vbo_(other.normal_vbo_), ibo_(other.ibo_),
      vertex_count_(other.vertex_count_), face_count_(other.face_count_) {
...
void MeshRenderData::upload_mesh_data(const core::Mesh &mesh) {
  ...
  glBufferData(GL_ARRAY_BUFFER, mesh.vertices().rows() * 3 * sizeof(double),
  ...
  glBufferData(GL_ARRAY_BUFFER, normals.rows() * 3 * sizeof(double),
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh.faces().rows() * 3 * sizeof(int),
               mesh.faces().data(), GL_STATIC_DRAW);

  unbind();
}

void MeshRenderData::bind() const { glBindVertexArray(vao_); }

void MeshRenderData::render(RenderMode mode) const {
  switch (mode) {
  case RenderMode::Wireframe:
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(face_count_ * 3),
...
void MeshRenderData::unbind() const { glBindVertexArray(0); }

// ViewportRenderer Implementation
ViewportRenderer::ViewportRenderer() = default;

ViewportRenderer::~ViewportRenderer() { shutdown(); }
...
GLuint ViewportRenderer::link_program(GLuint vertex_shader,
...
int ViewportRenderer::add_mesh(const core::Mesh &mesh,
...
void ViewportRenderer::clear_meshes() { mesh_cache_.clear(); }

void ViewportRenderer::render_mesh(int mesh_id,
  ...
  const float aspect_ratio = static_cast<float>(viewport_width_) /
...
bool ViewportRenderer::create_framebuffer() {
  ...
  glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, viewport_width_,
                        viewport_height_);
  ...
  glBindFramebuffer(GL_FRAMEBUFFER, 0);
  std::cout << "✅ Framebuffer created successfully (" << viewport_width_ << "x"
