...
std::unique_ptr<ComputeDevice::ComputeShader> GPUMeshGenerator::cylinder_shader_;
...
std::optional<core::Mesh> GPUMeshGenerator::generate_sphere(double radius, int u_segments, int v_segments) {
    if (!is_available() || !sphere_shader_) {
        set_last_error(core::ErrorUtils::make_error(
            core::ErrorCategory::GPU, core::ErrorCode::UnsupportedOperation,
            "GPU sphere generation not available", "GPUMeshGenerator::generate_sphere"));
        return std::nullopt;
    }
...
std::optional<core::Mesh> GPUMeshGenerator::generate_box(double width, double height, double depth,
    ...
    const size_t face_vertices = (width_segments + 1) * (height_segments + 1) * 2 + 
    ...
    const auto work_groups_x = (std::max({width_segments, height_segments, depth_segments}) + 15) / 16;
    box_shader_->dispatch(work_groups_x, work_groups_x, NUM_BOX_FACES);
    box_shader_->memory_barrier();
    
    return buffer_to_mesh(*vertex_buffer, *index_buffer, face_vertices, face_triangles);
}

std::optional<core::Mesh> GPUMeshGenerator::generate_plane(double width, double height, 
                                                            int width_segments, int height_segments) {
    if (!is_available() || !plane_shader_) {
        set_last_error(core::ErrorUtils::make_error(
            core::ErrorCategory::GPU, core::ErrorCode::UnsupportedOperation,
            "GPU plane generation not available", "GPUMeshGenerator::generate_plane"));
        return std::nullopt;
    }
...
std::optional<core::Mesh> GPUMeshGenerator::generate_cylinder(double radius, double height, 
                                                              int radial_segments, int height_segments, 
                                                              bool open_ended) {
    if (!is_available() || !cylinder_shader_) {
        set_last_error(core::ErrorUtils::make_error(
            core::ErrorCategory::GPU, core::ErrorCode::UnsupportedOperation,
            "GPU cylinder generation not available", "GPUMeshGenerator::generate_cylinder"));
        return std::nullopt;
    }
    ...
    const size_t cap_vertices = open_ended ? 0 : (radial_segments + 1) * 2; // top + bottom caps
    const size_t total_vertices = side_vertices + cap_vertices;
    
    const size_t side_faces = radial_segments * height_segments * 2;
    const size_t cap_faces = open_ended ? 0 : radial_segments * 2; // top + bottom caps
    ...
    return buffer_to_mesh(*vertex_buffer, *index_buffer, total_vertices, total_faces);
}

std::optional<core::Mesh> GPUMeshGenerator::generate_torus(double major_radius, double minor_radius,
                                                           int major_segments, int minor_segments) {
    if (!is_available() || !torus_shader_) {
        set_last_error(core::ErrorUtils::make_error(
            core::ErrorCategory::GPU, core::ErrorCode::UnsupportedOperation,
            "GPU torus generation not available", "GPUMeshGenerator::generate_torus"));
        return std::nullopt;
    }
...
std::string GPUMeshGenerator::get_performance_stats() {
    ...
    return "GPU Mesh Generation: Active\n"
           ...
           "Max Work Group Size: " + std::to_string(ComputeDevice::get_max_work_group_invocations());
}

const core::Error& GPUMeshGenerator::last_error() {
    return last_error_;
}
...
std::optional<core::Mesh> GPUMeshGenerator::buffer_to_mesh(
    ...
    const_cast<ComputeDevice::Buffer&>(vertex_buffer).download(vertices.data(), vertices.size() * sizeof(float));
    
    // Download index data (remove const for download)
    std::vector<unsigned int> indices(num_faces * 3);
    const_cast<ComputeDevice::Buffer&>(index_buffer).download(indices.data(), indices.size() * sizeof(unsigned int));
