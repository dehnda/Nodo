...
void ComputeDevice::ComputeShader::dispatch(GLuint num_groups_x, GLuint num_groups_y, GLuint num_groups_z) {
    if (program_id_ != 0) {
...
void ComputeDevice::ComputeShader::set_uniform(const std::string& name, int value) {
...
void ComputeDevice::ComputeShader::set_uniform(const std::string& name, float value) {
    GLint location = get_uniform_location(name);
    if (location != -1) {
...
void ComputeDevice::ComputeShader::set_uniform(const std::string& name, const std::vector<float>& values) {
    GLint location = get_uniform_location(name);
...
GLint ComputeDevice::ComputeShader::get_uniform_location(const std::string& name) {
...
std::unique_ptr<ComputeDevice::Buffer> ComputeDevice::create_buffer(size_t size, GLenum usage) {
    if (!is_available()) {
        return nullptr;
    }
...
std::unique_ptr<ComputeDevice::ComputeShader> ComputeDevice::create_shader(const std::string& source) {
    if (!is_available()) {
        return nullptr;
    }
...
std::unique_ptr<ComputeDevice::ComputeShader> ComputeDevice::load_shader_from_file(const std::string& filename) {
...
const core::Error& ComputeDevice::last_error() {
    return last_error_;
}
...
double GPUProfiler::Timer::get_elapsed_ms() {
    ...
    return static_cast<double>(end_time - start_time) / 1000000.0; // Convert to milliseconds
}
