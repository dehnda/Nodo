...
bool GLContext::initialize(int width, int height, bool visible) {
    ...
    // Create window
    window_ = glfwCreateWindow(width, height, "NodeFlux GPU Context", nullptr, nullptr);
    if (!window_) {
        set_last_error(core::ErrorUtils::make_error(
            core::ErrorCategory::GPU, core::ErrorCode::InitializationFailed,
...
std::string GLContext::get_context_info() {
    ...
    info << "  Compute Shaders: " << (GLEW_ARB_compute_shader ? "Supported" : "Not Supported") << "\n";
...
const core::Error& GLContext::last_error() {
    return last_error_;
}
