...
bool AttributeData::holds_type(const std::type_info& type) const {
  ...
  return std::visit(
      [&type](const auto& value) { return typeid(value) == type; }, data_[0]);
}
...
template <typename T>
void GeometryAttributes::add_attribute(const std::string& name,
                                       AttributeClass class_type,
                                       size_t initial_size) {
  auto attribute_data =
      std::make_unique<AttributeData>(class_type, initial_size);
...
GeometryAttributes::get_attribute_class(const std::string& name) const {
...
template <typename T>
bool GeometryAttributes::set_attribute(const std::string& name, size_t index,
                                       const T& value) {
...
template <typename T>
std::optional<T> GeometryAttributes::get_attribute(const std::string& name,
                                                   size_t index) const {
  const auto* data = get_attribute_data(name);
  if (data != nullptr) {
...
template <typename T>
bool GeometryAttributes::set_attribute_array(const std::string& name,
...
GeometryAttributes::get_attribute_array(const std::string& name) const {
  const auto* data = get_attribute_data(name);
  if (data != nullptr) {
...
bool GeometryAttributes::set_position(size_t vertex_index,
                                      const Vector3& position) {
  return set_attribute("position", vertex_index, position);
}

bool GeometryAttributes::set_normal(size_t vertex_index,
...
bool GeometryAttributes::set_uv_coordinates(size_t vertex_index,
                                            const Vector2f& uv_coords) {
  return set_attribute("uv", vertex_index, uv_coords);
}

std::optional<Vector3>
GeometryAttributes::get_position(size_t vertex_index) const {
  return get_attribute<Vector3>("position", vertex_index);
}

std::optional<Vector3>
GeometryAttributes::get_normal(size_t vertex_index) const {
  return get_attribute<Vector3>("normal", vertex_index);
}

std::optional<Vector3>
GeometryAttributes::get_color(size_t vertex_index) const {
  return get_attribute<Vector3>("color", vertex_index);
}

std::optional<Vector2f>
GeometryAttributes::get_uv_coordinates(size_t vertex_index) const {
...
void GeometryAttributes::transfer_attributes(
    const GeometryAttributes& source, const std::vector<int>& vertex_mapping,
  ...
  if (!vertex_mapping.empty()) {
    for (const auto& attr_name :
         source.get_attribute_names(AttributeClass::VERTEX)) {
      ...
      if (!has_attribute(attr_name)) {
        if (source_data->holds_type<Vector3>()) {
          add_attribute<Vector3>(attr_name, AttributeClass::VERTEX,
                                 vertex_mapping.size());
        } else if (source_data->holds_type<Vector2f>()) {
          add_attribute<Vector2f>(attr_name, AttributeClass::VERTEX,
                                  vertex_mapping.size());
        } else if (source_data->holds_type<float>()) {
          add_attribute<float>(attr_name, AttributeClass::VERTEX,
                               vertex_mapping.size());
        }
      }
  ...
  if (!face_mapping.empty()) {
    for (const auto& attr_name :
         source.get_attribute_names(AttributeClass::FACE)) {
      const auto* source_data = source.get_attribute_data(attr_name);
      if (source_data == nullptr)
        continue;
      ...
      if (source_data->holds_type<int>()) {
        add_attribute<int>(attr_name, AttributeClass::FACE,
                           face_mapping.size());
      }
    }
  }
...
bool GeometryAttributes::promote_vertex_to_face(
    const std::string& vertex_attr_name, const std::string& face_attr_name,
    const std::vector<Vector3i>& faces) {
  const auto* vertex_data = get_attribute_data(vertex_attr_name);
  if (vertex_data == nullptr ||
      vertex_data->get_class() != AttributeClass::VERTEX) {
    return false;
  }
  ...
  if (vertex_data->holds_type<Vector3>()) {
    ...
    for (size_t face_idx = 0; face_idx < faces.size(); ++face_idx) {
      ...
      if (vertex_0_value.has_value() && vertex_1_value.has_value() &&
          vertex_2_value.has_value()) {
        Vector3 face_value = (vertex_0_value.value() + vertex_1_value.value() +
                              vertex_2_value.value()) /
...
bool GeometryAttributes::demote_face_to_vertex(
    const std::string& face_attr_name, const std::string& vertex_attr_name,
  ...
  if (face_data->holds_type<Vector3>()) {
    add_attribute<Vector3>(vertex_attr_name, AttributeClass::VERTEX,
                           vertex_count);
    ...
    for (size_t vertex_idx = 0; vertex_idx < vertex_count; ++vertex_idx) {
      if (vertex_counts[vertex_idx] > 0) {
        vertex_values[vertex_idx] /=
            static_cast<double>(vertex_counts[vertex_idx]);
      }
...
void GeometryAttributes::resize_attributes(AttributeClass class_type,
                                           size_t new_size) {
...
size_t
GeometryAttributes::get_attribute_count(AttributeClass class_type) const {
  size_t count = 0;
...
GeometryAttributes::get_attribute_names(AttributeClass class_type) const {
...
void GeometryAttributes::initialize_standard_attributes(size_t vertex_count,
                                                        size_t face_count) {
  ...
  std::vector<Vector3> default_colors(
      vertex_count, Vector3(DEFAULT_COLOR_VALUE, DEFAULT_COLOR_VALUE,
                            DEFAULT_COLOR_VALUE)); // Light gray
...
void GeometryAttributes::ensure_attribute_exists(const std::string& name,
                                                 AttributeClass class_type,
                                                 size_t size) {
...
AttributeData* GeometryAttributes::get_attribute_data(const std::string& name) {
  auto attribute_iterator = attributes_.find(name);
  return (attribute_iterator != attributes_.end())
             ? attribute_iterator->second.get()
             : nullptr;
}

const AttributeData*
GeometryAttributes::get_attribute_data(const std::string& name) const {
  auto attribute_iterator = attributes_.find(name);
  return (attribute_iterator != attributes_.end())
             ? attribute_iterator->second.get()
             : nullptr;
...
template std::optional<Vector2f>
    AttributeData::get_value<Vector2f>(size_t) const;
...
template std::optional<std::string>
    AttributeData::get_value<std::string>(size_t) const;
...
template void GeometryAttributes::add_attribute<Vector3>(const std::string&,
                                                         AttributeClass,
                                                         size_t);
template void GeometryAttributes::add_attribute<Vector2f>(const std::string&,
                                                          AttributeClass,
                                                          size_t);
template void GeometryAttributes::add_attribute<float>(const std::string&,
                                                       AttributeClass, size_t);
template void GeometryAttributes::add_attribute<int>(const std::string&,
                                                     AttributeClass, size_t);
template void GeometryAttributes::add_attribute<std::string>(const std::string&,
                                                             AttributeClass,
                                                             size_t);

template bool GeometryAttributes::set_attribute<Vector3>(const std::string&,
                                                         size_t,
                                                         const Vector3&);
template bool GeometryAttributes::set_attribute<Vector2f>(const std::string&,
                                                          size_t,
                                                          const Vector2f&);
template bool GeometryAttributes::set_attribute<float>(const std::string&,
                                                       size_t, const float&);
template bool GeometryAttributes::set_attribute<int>(const std::string&, size_t,
                                                     const int&);
template bool
GeometryAttributes::set_attribute<std::string>(const std::string&, size_t,
                                               const std::string&);

template std::optional<Vector3>
GeometryAttributes::get_attribute<Vector3>(const std::string&, size_t) const;
template std::optional<Vector2f>
GeometryAttributes::get_attribute<Vector2f>(const std::string&, size_t) const;
template std::optional<float>
GeometryAttributes::get_attribute<float>(const std::string&, size_t) const;
template std::optional<int>
GeometryAttributes::get_attribute<int>(const std::string&, size_t) const;
template std::optional<std::string>
GeometryAttributes::get_attribute<std::string>(const std::string&,
                                               size_t) const;
