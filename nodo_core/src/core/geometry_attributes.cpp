...
AttributeData::AttributeData(AttributeClass class_type, size_t size) : class_(class_type) {
  data_.resize(size);
}
...
bool AttributeData::holds_type(const std::type_info& type) const {
  ...
  return std::visit([&type](const auto& value) { return typeid(value) == type; }, data_[0]);
}
...
void GeometryAttributes::transfer_attributes(const GeometryAttributes& source, const std::vector<int>& vertex_mapping,
...
bool GeometryAttributes::promote_vertex_to_face(const std::string& vertex_attr_name, const std::string& face_attr_name,
  ...
  if (vertex_data->holds_type<Vector3>()) {
    ...
    for (size_t face_idx = 0; face_idx < faces.size(); ++face_idx) {
      ...
      if (vertex_0_value.has_value() && vertex_1_value.has_value() && vertex_2_value.has_value()) {
        Vector3 face_value =
            (vertex_0_value.value() + vertex_1_value.value() + vertex_2_value.value()) / TRIANGLE_VERTEX_COUNT;
...
bool GeometryAttributes::demote_face_to_vertex(const std::string& face_attr_name, const std::string& vertex_attr_name,
...
void GeometryAttributes::initialize_standard_attributes(size_t vertex_count, size_t face_count) {
  ...
  std::vector<Vector3> default_colors(vertex_count, Vector3(DEFAULT_COLOR_VALUE, DEFAULT_COLOR_VALUE,
