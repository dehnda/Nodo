...
thread_local core::Error SphereGenerator::last_error_{core::ErrorCategory::Unknown, core::ErrorCode::Unknown,
                                                      "No error"};
...
std::optional<core::GeometryContainer> SphereGenerator::generate_uv_sphere(double radius, int u_segments,
                                                                           int v_segments) {
  if (radius <= 0.0) {
    set_last_error(
        core::Error{core::ErrorCategory::Validation, core::ErrorCode::InvalidFormat, "Sphere radius must be positive"});
    return std::nullopt;
  }
  ...
  for (int ring = 1; ring < v_segments; ++ring) {
    const double phi = std::numbers::pi * static_cast<double>(ring) / static_cast<double>(v_segments);
    ...
    for (int segment = 0; segment < u_segments; ++segment) {
      const double theta = 2.0 * std::numbers::pi * static_cast<double>(segment) / static_cast<double>(u_segments);
      const double coord_x = ring_radius * std::cos(theta);
      const double coord_z = ring_radius * std::sin(theta);

      positions.push_back({static_cast<float>(coord_x), static_cast<float>(coord_y), static_cast<float>(coord_z)});
    }
  }
...
std::optional<core::GeometryContainer> SphereGenerator::generate_icosphere(double radius, int subdivisions) {
  if (radius <= 0.0) {
    set_last_error(
        core::Error{core::ErrorCategory::Validation, core::ErrorCode::InvalidFormat, "Sphere radius must be positive"});
    return std::nullopt;
  ...
  std::vector<Eigen::Vector3d> vertices = {
      {-icosa_scale, golden_ratio * icosa_scale, 0},  {icosa_scale, golden_ratio * icosa_scale, 0},
      {-icosa_scale, -golden_ratio * icosa_scale, 0}, {icosa_scale, -golden_ratio * icosa_scale, 0},
      {0, -icosa_scale, golden_ratio * icosa_scale},  {0, icosa_scale, golden_ratio * icosa_scale},
      {0, -icosa_scale, -golden_ratio * icosa_scale}, {0, icosa_scale, -golden_ratio * icosa_scale},
      {golden_ratio * icosa_scale, 0, -icosa_scale},  {golden_ratio * icosa_scale, 0, icosa_scale},
      {-golden_ratio * icosa_scale, 0, -icosa_scale}, {-golden_ratio * icosa_scale, 0, icosa_scale}};

  // Initial icosahedron faces - these indices define the icosahedron topology
  std::vector<std::array<int, 3>> faces = {{0, 11, 5}, {0, 5, 1},  {0, 1, 7},   {0, 7, 10}, {0, 10, 11},
  ...
  for (int level = 0; level < subdivisions; ++level) {
    ...
    for (const auto& face : faces) {
      ...
      auto get_midpoint = [&](int vertex1, int vertex2) -> int {
        ...
        Eigen::Vector3d midpoint = (vertices[vertex1] + vertices[vertex2]) * 0.5;
  ...
  if (p_storage != nullptr) {
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
      ...
      p_span[i] = {static_cast<float>(scaled_vertex.x()), static_cast<float>(scaled_vertex.y()),
  ...
  if (n_storage != nullptr) {
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
      ...
      n_span[i] = {static_cast<float>(vertices[i].x()), static_cast<float>(vertices[i].y()),
...
Eigen::Vector3d SphereGenerator::normalize_vertex(const Eigen::Vector3d& vertex, double radius) {
