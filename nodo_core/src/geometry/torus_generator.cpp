...
thread_local core::Error TorusGenerator::last_error_{core::ErrorCategory::Geometry, core::ErrorCode::Unknown, ""};

std::optional<core::GeometryContainer> TorusGenerator::generate(double major_radius, double minor_radius,
  ...
  for (int major_idx = 0; major_idx < major_segments; ++major_idx) {
    const double major_angle = static_cast<double>(major_idx) * major_angle_step;
    ...
    for (int minor_idx = 0; minor_idx < minor_segments; ++minor_idx) {
      const double minor_angle = static_cast<double>(minor_idx) * minor_angle_step;
      ...
      positions.push_back({static_cast<float>(pos_x), static_cast<float>(pos_y), static_cast<float>(pos_z)});
      ...
      normals.push_back({static_cast<float>(normal_x), static_cast<float>(normal_y), static_cast<float>(normal_z)});
    }
  }
