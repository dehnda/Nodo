...
public:
   ...
   */
  void set_iterations(int iterations) { set_parameter("iterations", iterations); }
   ...
   */
  void set_lambda(double lambda) { set_parameter("lambda", static_cast<float>(lambda)); }
   ...
   */
  void set_method(SmoothingMethod method) { set_parameter("method", static_cast<int>(method)); }

  /**
   * @brief Get smoothing method
   */
  SmoothingMethod get_method() const { return static_cast<SmoothingMethod>(get_parameter<int>("method", 0)); }
...
private:
  ...
  Eigen::MatrixXd apply_uniform_laplacian(const Eigen::MatrixXd& vertices, const Eigen::MatrixXi& faces);

  /// Apply cotangent-weighted Laplacian smoothing
  Eigen::MatrixXd apply_cotangent_laplacian(const Eigen::MatrixXd& vertices, const Eigen::MatrixXi& faces);

  /// Apply Taubin smoothing (lambda-mu scheme)
  Eigen::MatrixXd apply_taubin_smoothing(const Eigen::MatrixXd& vertices, const Eigen::MatrixXi& faces);

  /// Build uniform Laplacian matrix
  Eigen::SparseMatrix<double> build_uniform_laplacian(const Eigen::MatrixXi& faces, int num_vertices);

  /// Build cotangent-weighted Laplacian matrix
  Eigen::SparseMatrix<double> build_cotangent_laplacian(const Eigen::MatrixXd& vertices, const Eigen::MatrixXi& faces);

  /// Find boundary vertices
  std::vector<bool> find_boundary_vertices(const Eigen::MatrixXi& faces, int num_vertices);

  /// Calculate cotangent weights for an edge
  double calculate_cotangent_weight(const Eigen::Vector3d& v1, const Eigen::Vector3d& v2,
