...
public:
  ...
  explicit RemeshingSOP(const std::string& node_name = "remesh") : SOPNode(node_name, "Remesh") {
    // Single input
    // Output is defined in base class

    // Remeshing mode
    register_parameter(define_int_parameter("mode", 0, {"Uniform", "Adaptive"})
                           .label("Mode")
                           .category("Remeshing")
    ...
    register_parameter(define_float_parameter("target_edge_length", 1.0F)
    ...
    register_parameter(define_float_parameter("min_edge_length", 0.1F)
    ...
    register_parameter(define_float_parameter("max_edge_length", 2.0F)
    ...
    register_parameter(define_float_parameter("approx_error", 0.01F)
...
protected:
  std::shared_ptr<core::GeometryContainer> execute() override {
    ...
    params.target_edge_length = get_parameter<float>("target_edge_length", 1.0F);
    ...
    params.smoothing_iterations = get_parameter<int>("smoothing_iterations", 10);
    params.preserve_boundaries = get_parameter<bool>("preserve_boundaries", true);
