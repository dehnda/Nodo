...
public:
  explicit BoxSOP(const std::string& node_name = "box") : SOPNode(node_name, "Box") {
...
protected:
  core::Result<std::shared_ptr<core::GeometryContainer>> execute() override {
    ...
    const auto width_segments = get_parameter<int>("width_segments", DEFAULT_SEGMENTS);
    const auto height_segments = get_parameter<int>("height_segments", DEFAULT_SEGMENTS);
    const auto depth_segments = get_parameter<int>("depth_segments", DEFAULT_SEGMENTS);
    const auto primitive_type = static_cast<PrimitiveType>(get_parameter<int>("primitive_type", 0));

    try {
      auto result =
          geometry::BoxGenerator::generate(static_cast<double>(width), static_cast<double>(height),
