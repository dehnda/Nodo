...
public:
   ...
   */
  void set_target_percentage(float percentage) {
    set_parameter("target_percentage", percentage);
  }
   ...
   */
  void set_target_vertex_count(int count) {
    set_parameter("target_vertex_count", count);
  }
   ...
   * target_percentage
   */
  void set_use_vertex_count(bool use_count) {
    set_parameter("use_vertex_count", use_count);
  }
   ...
   */
  void set_preserve_topology(bool preserve) {
    set_parameter("preserve_topology", preserve);
  }
  ...
  void set_preserve_boundaries(bool preserve) {
    set_parameter("preserve_boundaries", preserve);
  }

  // Node metadata
  std::string get_category() const { return "Modify"; }
  std::string get_description() const {
    return "Reduce mesh complexity while preserving shape";
  }
